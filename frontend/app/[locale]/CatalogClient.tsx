...
export type Project = {slug: string; title: string; summary?: string; department: string; chair: string; opportunity_type: "project_study" | "idp"; language?: string; topics: string[]; freshness: string; source_url: string; artifact_url?: string; published_at?: string};
...
type Copy = {brand: string; title: string; lede: string; notice: string; search: string; filters: string; lastUpdated: string; age: string; under3: string; under30: string; under60: string; under180: string; over180: string; type: string; display: string; all: string; previous: string; next: string; foundOne: string; foundMany: string; source: string; artifact: string; unknown: string; published: string; current: string; old: string; undated: string};
...
export default function CatalogClient({lang, copy, initialProjects, lastUpdatedAt, departments, chairs}: {lang: "en" | "de"; copy: Copy; initialProjects: Project[]; lastUpdatedAt?: string; departments: Department[]; chairs: Chair[]}) {
  ...
  const displayedProjects = useMemo(() => {
    ...
    return initialProjects.filter((project) => {
      ...
      const daysOld = project.published_at ? Math.floor((Date.now() - Date.parse(`${project.published_at}T00:00:00Z`)) / 86_400_000) : null;
      const ageBand = daysOld === null ? null : daysOld < 3 ? "lt3" : daysOld < 30 ? "lt30" : daysOld < 60 ? "lt60" : daysOld < 180 ? "lt180" : "gt180";
      const matchesAge = selectedAgeBands.size === 0 || (ageBand !== null && selectedAgeBands.has(ageBand));
  ...
  }, [initialProjects, query, selectedAgeBands, selectedChairs, selectedTypes]);
  ...
  return <>
    ...
    <main><section className="hero"><div className="shell"><h1>{copy.title}</h1><p className="lede">{copy.lede}</p><p className="notice">{copy.notice}</p></div></section><div className="shell catalog">
      ...
      <section aria-live="polite"><div className="results-head"><input className="search" value={query} onChange={(event) => { setQuery(event.target.value); setCurrentPage(1); }} type="search" placeholder={copy.search} aria-label={copy.search}/><label>{copy.display} <select value={displayCount} onChange={(event) => { setDisplayCount(event.target.value); setCurrentPage(1); }} aria-label={copy.display}><option value="10">10</option><option value="20">20</option><option value="50">50</option><option value="100">100</option><option value="all">{copy.all}</option></select></label></div><div className="filter-row"><fieldset><legend>{copy.age}</legend>{ageBands.map((band) => <label key={band}><input type="checkbox" checked={selectedAgeBands.has(band)} onChange={(event) => toggleAgeBand(band, event.target.checked)}/>{ageLabels[band]}</label>)}</fieldset><fieldset><legend>{copy.type}</legend><label><input type="checkbox" checked={selectedTypes.has("project_study")} onChange={(event) => toggleType("project_study", event.target.checked)}/>Project Study</label><label><input type="checkbox" checked={selectedTypes.has("idp")} onChange={(event) => toggleType("idp", event.target.checked)}/>IDP</label></fieldset></div><p><strong>{displayedProjects.length}</strong> {displayedProjects.length === 1 ? copy.foundOne : copy.foundMany}</p>{visibleProjects.map((project) => <article className="card" key={project.slug}><p className="meta">{project.department} · {project.chair}</p><h2>{project.title}</h2><p>{project.summary || copy.unknown}</p><div className="tags"><span className="tag">{project.opportunity_type === "idp" ? "IDP" : "Project Study"}</span>{project.language && <span className="tag">{project.language}</span>}{project.topics.map((topic) => <span className="tag" key={topic}>{topic.replaceAll("_", " / ")}</span>)}<span className="tag">{project.published_at ? `${copy.published} ${date(project.published_at)}` : copy.undated}</span></div><p className="links"><a className="source" href={project.source_url} target="_blank" rel="noopener noreferrer">{copy.source} ↗</a>{project.artifact_url && <a className="source" href={project.artifact_url} target="_blank" rel="noopener noreferrer">{copy.artifact} ↗</a>}</p></article>)}{displayCount !== "all" && pageCount > 1 && <nav className="pagination" aria-label="Pagination"><button type="button" onClick={() => setCurrentPage(activePage - 1)} disabled={activePage === 1}>{copy.previous}</button>{Array.from({length: pageCount}, (_, index) => index + 1).map((page) => <button type="button" key={page} onClick={() => setCurrentPage(page)} aria-current={page === activePage ? "page" : undefined}>{page}</button>)}<button type="button" onClick={() => setCurrentPage(activePage + 1)} disabled={activePage === pageCount}>{copy.next}</button></nav>}</section>
    </div></main>
