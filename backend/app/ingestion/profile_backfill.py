...
from .project_profile import SCHEMA_VERSION, PdfTextCoverage, ProfileExtraction
...
def _store(result: ProfileExtraction) -> None:
    ...
    with session_factory()() as session:
        session.add(PDFProfileExtraction(**values))
        session.commit()
...
def main() -> None:
    ...
    config = settings()
    if not args.dry_run and not (config.azure_openai_api_key and config.azure_openai_endpoint):
